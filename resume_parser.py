...
def call_llm(system_prompt: str, user_prompt: str) -> str:
    """调用阿里云百炼兼容接口（OpenAI SDK 风格），返回模型的纯文本响应。"""
    ...
    completion = client.chat.completions.create(
        ...
        temperature=0.0,  # 抽取任务用低温度，保证结构稳定
    )
...
def safe_json_parse(raw: str) -> Optional[dict]:
    """容错地解析 LLM 输出为 JSON。失败时打印清晰错误并返回 None。"""
    if not raw or not raw.strip():
        print("[ERROR] LLM 输出为空字符串")
        return None

    # 1) 去 Markdown 包装后直接解析
    ...
    print(f"[ERROR] LLM 输出中未发现 JSON 结构，原始输出前 500 字符：\n{raw[:500]}")
    return None
